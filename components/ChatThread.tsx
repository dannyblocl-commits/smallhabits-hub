...
export function ChatThread({ initial, peerId, peerName, canSend, memberId, accent = "fucsia" }: { initial: Msg[]; peerId: string | null; peerName: string; canSend: boolean; memberId?: string; accent?: "fucsia" | "sage" }) {
  ...
  useEffect(() => {
    const id = setInterval(() => { listThread(memberId).then((t) => setMsgs(t.messages)).catch(() => {}); }, 8000);
  ...
  function send() {
    ...
    setErr(""); setText("");
    start(async () => {
      try { const m = await sendMessage(body, memberId); setMsgs((l) => [...l, m]); }
      catch (e) { setErr(e instanceof Error ? e.message : "No se pudo enviar."); setText(body); }
    });
  }
  ...
  return (
    <div className="card h-[520px] flex flex-col">
      <div ref={box} className="flex-1 overflow-y-auto p-4 space-y-3">
        {!peerId && <div className="row p-4 text-sm muted text-center">Todavía no hay una coach en la plataforma. En cuanto entre, tu mensaje le llega.</div>}
        {peerId && msgs.length === 0 && <div className="row p-4 text-sm muted text-center">Escribe el primer mensaje a {peerName}.</div>}
        {msgs.map((m) => (
      ...
      <div className="p-3" style={{ borderTop: "1px solid var(--line)" }}>
        ...
        <div className="flex gap-2">
          <input id={`msg-${memberId ?? "coach"}`} value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} disabled={!canSend || pending}
            placeholder={canSend ? `Escribe a ${peerName}…` : "Disponible desde el plan Pro"} className={`input ${accent === "sage" ? "input-s" : ""}`} />
          <button onClick={send} disabled={!canSend || pending} className={`btn btn-sm ${accent === "sage" ? "btn-balance" : "btn-go"}`}>Enviar</button>
        </div>
      </div>
    </div>
  );
