...
export type User = { id: string; email: string; name: string; goal: string; weight: number | null; height: number | null; plan: Plan; role: Role; coach_id: string | null; created_at: string; subscription_id: string | null; trial_end: string | null; reto_start: string | null; reto_track: string | null };
...
export async function getUser(): Promise<User | null> {
  ...
  try {
    ...
    await ensureSchema();
    const r = await db().query("select id, email, name, goal, weight, height, plan, role, coach_id, created_at, subscription_id, trial_end, reto_start, reto_track from users where id = $1", [payload.uid]);
