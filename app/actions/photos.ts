...
export async function listPhotos(userId?: string): Promise<Photo[]> {
  ...
  const r = await db().query("select id, url, kind, weight, note, at from photos where user_id=$1 order by at asc", [target]);
  return r.rows.map((p) => ({ ...p, weight: p.weight === null ? null : Number(p.weight), at: new Date(p.at).toISOString() }));
}
...
export async function uploadPhoto(form: FormData) {
  ...
  const pathname = `photos/${me.id}/${Date.now()}.${ext}`;
...
export async function deletePhoto(form: FormData) {
  ...
  const r = await db().query("delete from photos where id=$1 and user_id=$2 returning url", [id, me.id]);
  if (r.rows[0]) await del(r.rows[0].url).catch(() => {});
  redirect("/dashboard/photos");
}
